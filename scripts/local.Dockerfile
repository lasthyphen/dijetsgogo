...
# in order to build an image using the local version of dijethh
...
COPY dijethh dijethh

WORKDIR $GOPATH/src/github.com/lasthyphen/dijetsgogo
RUN ./scripts/build_avalanche.sh
RUN ./scripts/build_dijethh.sh ../dijethh $PWD/build/plugins/evm
